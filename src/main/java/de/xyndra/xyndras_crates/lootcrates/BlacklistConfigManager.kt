...
class BlacklistConfigManager {
    private val blacklistPath: Path = Paths.get("config/xyndras-crate/blacklist.txt")
    ...
    fun getBlacklist(): Set<WorldBlockPos> {
        ...
        if (needsMigration) {
            println("[xyndra-Crates] Migrating blacklist to world-aware format...")
            saveBlacklist(blacklist)
            println("[xyndra-Crates] Blacklist migration complete! ${blacklist.size} entries migrated to overworld.")
        }
