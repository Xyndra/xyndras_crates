...
class CrateDataManager {
    private val CRATE_DATA_FILE = Paths.get("config", "xyndras-crate", "crate_data.json").toString()
    ...
    fun loadCrateData(): Map<WorldBlockPos, String> {
        ...
        if (needsMigration) {
            println("[xyndra-Crates] Migrating crate data to world-aware format...")
            saveCrateData(crateData)
            println("[xyndra-Crates] Migration complete! ${crateData.size} crates migrated to overworld.")
        }
